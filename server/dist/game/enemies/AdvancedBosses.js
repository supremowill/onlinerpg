...
exports.TheMightyOneEnemy = exports.CaoDosInfernosEnemy = exports.PlantaCarnivoraEnemy = exports.LichKingEnemy = exports.FeiticeiroImortalEnemy = void 0;
...
class FeiticeiroImortalEnemy extends Enemy_1.ServerEnemy {
    baseDamage;
    idealDistance = 18;
    retreatDistance = 12;
    teleportTimer = 15000;
    isImmortal = false;
    souls = []; // soul enemy IDs
    habilidades = {
    ...
    pendingProjectiles = [];
    pendingAbilities = [];
    constructor(pos, globalMult, playerLevel, playerMaxHp) {
        super(pos);
        this.type = 'FeiticeiroImortal';
        this.name = 'Lord Vouldemord';
        this.maxHp = 50000 + (1000 * playerLevel);
        this.hp = this.maxHp;
        this.baseDamage = 500 + (playerMaxHp * 0.8);
        this.speed = 1.2;
        this.originalSpeed = 1.2;
        this.xp = config_1.CONFIG.FEITICEIRO_IMORTAL.XP;
        this.score = config_1.CONFIG.FEITICEIRO_IMORTAL.SCORE;
        this.hitboxRadius = config_1.CONFIG.FEITICEIRO_IMORTAL.HITBOX_RADIUS;
    ...
    takeDamage(amount, instigator, countsForPassive = true) {
        ...
        if (b.isActive && b.shieldHp > 0) {
            const ds = Math.min(amount, b.shieldHp);
            b.shieldHp -= ds;
            // Reflect 40% damage
            if (instigator) {
    ...
    update(dt, players, gameTime) {
        ...
        if (this.teleportTimer <= 0) {
            ...
            this.position.add(new Vector3_1.Vec3(Math.cos(angle) * 10, 0, Math.sin(angle) * 10));
        }
        if (b.isActive) {
            b.timer -= ms;
            if (b.timer <= 0)
        ...
        if (now > this.habilidades.ritual.lastUsed + this.habilidades.ritual.cooldown && this.souls.length === 0) {
            this.habilidades.ritual.lastUsed = now;
...
exports.FeiticeiroImortalEnemy = FeiticeiroImortalEnemy;
/** LichKing - frost aura, frost cone, prison, skull summon, blizzard ultimate, revival queue */
class LichKingEnemy extends Enemy_1.ServerEnemy {
    auraRadius;
    revivalQueue = [];
    habilidades = {
    ...
    constructor(pos, globalMult, playerLevel, playerMaxHp) {
        ...
        this.type = 'LichKing';
        this.name = 'Lich King';
        const c = config_1.CONFIG.LICH_KING;
        this.maxHp = 32000 + (450 * playerLevel) + (playerMaxHp * 0.7);
        this.hp = this.maxHp;
    ...
    addToRevivalQueue(position, originalType) {
    ...
    update(dt, players, gameTime) {
        ...
        if (this.habilidades.ultimate.isChanneling) {
            ...
            if (this.habilidades.ultimate.timer <= 0) {
                ...
                for (const p of players) {
                    if (!p.isDead) {
                        p.applySlow(4000, 0.5);
                        p.statusEffects.lichKingLifeDrain.isActive = true;
                        p.statusEffects.lichKingLifeDrain.timer = 4000;
                        p.statusEffects.lichKingLifeDrain.damagePerSecond = p.maxHp * 0.05;
                    }
                }
            }
            return;
        ...
        if (now > this.habilidades.ultimate.lastUsed + this.habilidades.ultimate.cooldown) {
            this.habilidades.ultimate.lastUsed = now;
...
exports.LichKingEnemy = LichKingEnemy;
/** PlantaCarnivoraRainha - whip, poison seeds, sprout summon, devour (pull + bite) */
class PlantaCarnivoraEnemy extends Enemy_1.ServerEnemy {
    habilidades = {
    ...
    constructor(pos, globalMult, playerMaxHp) {
        super(pos);
        this.type = 'PlantaCarnivora';
        this.name = 'Planta Carnívora Rainha';
        const c = config_1.CONFIG.PLANTA_CARNIVORA;
        this.maxHp = c.BASE_HP + (playerMaxHp * 0.8);
        this.hp = this.maxHp;
        this.speed = 0;
    ...
    update(dt, players, gameTime) {
        ...
        if (hd.isCharging) {
            ...
            for (const p of players) {
                ...
                if (this.position.distanceToXZ(p.position) < 15) {
                    const pull = this.position.clone().sub(p.position);
                    pull.y = 0;
                    pull.normalize();
        ...
        if (dist < 10 && now > hd.lastUsed + hd.cooldown) {
            hd.lastUsed = now;
            hd.isCharging = true;
            hd.chargeTimer = hd.chargeDuration;
        }
            ...
            for (let i = 0; i < 3; i++) {
                const offset = new Vector3_1.Vec3((Math.random() - 0.5) * 15, 0, (Math.random() - 0.5) * 15);
                const tgt = target.position.clone().add(offset);
                const dir = tgt.sub(this.position);
                dir.normalize();
...
exports.PlantaCarnivoraEnemy = PlantaCarnivoraEnemy;
/** CaoDosInfernos - dash, mark, roar+shield, frenzy, spawns filhotes */
class CaoDosInfernosEnemy extends Enemy_1.ServerEnemy {
    playerLevel;
    attackCooldown;
    lastAttackTime = 0;
    attackRange = 2.0;
    ...
    pendingAbilities = [];
    constructor(pos, globalMult, playerLevel) {
        super(pos);
        this.playerLevel = playerLevel;
        this.type = 'CaoDosInfernos';
        this.name = 'Cão dos Infernos';
        const c = config_1.CONFIG.CAO_DOS_INFERNOS;
        this.maxHp = c.BASE_HP + (playerLevel * c.BASE_HP_PER_LEVEL);
        this.hp = this.maxHp;
        this.damage = c.BASE_DAMAGE + (playerLevel * c.BASE_DAMAGE_PER_LEVEL);
        this.speed = c.SPEED;
        this.originalSpeed = c.SPEED;
        this.xp = 30 + (playerLevel * 10);
        this.score = 500 + (playerLevel * 20);
        this.hitboxRadius = c.HITBOX_RADIUS;
    ...
    update(dt, players, gameTime) {
        ...
        // Frenzy timer
        if (h.frenesi.isActive) {
            h.frenesi.timer -= ms;
            if (h.frenesi.timer <= 0) {
                h.frenesi.isActive = false;
                h.frenesi.lastUsed = now;
                h.investida.useCount = 0;
            }
        }
        else {
            if (this.updateStatus(dt))
                return;
        }
        // Mark timer
        if (h.marca.isActive) {
        ...
        // Mark
        if (now > h.marca.lastUsed + h.marca.cooldown) {
            h.marca.lastUsed = now;
            h.marca.isActive = true;
            h.marca.timer = h.marca.duration;
        }
        // Dash
        ...
        if (now > h.investida.lastUsed + dashCd && dist < 15 && !h.investida.isDashing) {
            h.investida.lastUsed = now;
            h.investida.isDashing = true;
            h.investida.dashTimer = h.investida.dashDuration;
            if (!h.frenesi.isActive)
                h.investida.useCount++;
        }
...
exports.CaoDosInfernosEnemy = CaoDosInfernosEnemy;
/** TheMightyOne - the ultimate boss, reflects projectiles, spawns elite minions, absorbs orbs */
class TheMightyOneEnemy extends Enemy_1.ServerEnemy {
    auraRadius;
    damageBonus = 1.0;
    ...
    constructor(pos) {
        super(pos);
        this.type = 'TheMightyOne';
        this.name = 'O Poderoso';
        const c = config_1.CONFIG.THE_MIGHTY_ONE;
        this.maxHp = c.HP;
        this.hp = this.maxHp;
        this.speed = c.SPEED;
        this.originalSpeed = c.SPEED;
...
exports.TheMightyOneEnemy = TheMightyOneEnemy;
//# sourceMappingURL=AdvancedBosses.js.map